...
if (!isset($_SESSION["awalBulan"])) {
    if (!isset($_SESSION["akhirBulan"])) {
...
$pemasukkan = query("SELECT * FROM pemasukan WHERE awal >= '$awal' AND awal <= '$akhir' ORDER BY awal");
$pengeluaran = query("SELECT * FROM pengeluaran WHERE tgl_cair >= '$awal' AND tgl_cair <= '$akhir' ORDER BY tgl_cair");
$anggota = query("SELECT * FROM mutasi WHERE tanggal >= '$awal' AND tanggal <= '$akhir' AND kode = 'Debit' ORDER BY tanggal");
$anggota2 = query("SELECT * FROM mutasi WHERE tanggal >= '$awal' AND tanggal <= '$akhir' AND kode = 'Kredit' ORDER BY tanggal");

$unionPemasukan = query("(SELECT awal AS tanggal, nama AS nama, nominal_akhir AS jumlah FROM pemasukan WHERE awal >= '$awal' AND awal <= '$akhir' ORDER BY awal)
                        UNION
...
$unionPengeluaran = query("(SELECT tgl_cair AS tanggal, nama AS nama, nominal_cair AS jumlah FROM pengeluaran WHERE tgl_cair >= '$awal' AND tgl_cair <= '$akhir' ORDER BY tgl_cair)
                        UNION
            ...
            <table class="table table-striped table-bordered table-sm col-md-6">
                <thead class="thead-dark">
                    ...
                    <tr>
                        ...
                        <th scope="col" style="width: 260px">Bulan</th>
                ...
                <tbody>
                    ...
                    <?php foreach ($unionPemasukan as $row) : ?>
                        ...
                        if ($bulanBefore == $bulanNow && $tahunBefore == $tahunNow) {
                            ...
                            <?php $jumlahSementara = $row["jumlah"]; ?>
                            <!-- <?php foreach ($anggota as $baris) :
                                        $bulanSekarang = explode("-", $baris["tanggal"])[1];
                                        $tahunSekarang = explode("-", $baris["tanggal"])[0];
                                        if ($baris["kode"] == "Debit" && $bulanBefore == $bulanSekarang && $tahunBefore == $tahunSekarang) {
                                            $angka = $baris["jumlah"];
                                            $pemasukanBulanan = $pemasukanBulanan + $angka;
                                        }
                                    endforeach ?> -->
                            <tr>
                            ...
                            <?php $i++;
                            $totalPemasukan = $totalPemasukan + $pemasukanBulanan;
                            $pemasukanBulanan = 0; ?>
                        <?php } ?>
                        <?php if ($x == sizeof($pemasukkan)) { ?>
                            <!-- <?php foreach ($anggota as $baris) :
                                        $bulanSekarang = explode("-", $baris["tanggal"])[1];
                                        $tahunSekarang = explode("-", $baris["tanggal"])[0];
                                        if ($baris["kode"] == "Debit" && $bulanNow == $bulanSekarang && $tahunNow == $tahunSekarang) {
                                            $angka = $baris["jumlah"];
                                            $pemasukanBulanan = $pemasukanBulanan + $angka;
                                        }
                                    endforeach ?> -->
                    ...
                    <?php foreach ($anggota as $baris) :
                        if ($bulanSekarang == null) {
                            $bulanSebelumnya = explode("-", $baris["tanggal"])[1];
                        } else {
                            $bulanSebelumnya = $bulanSekarang;
                        }
                        if ($tahunSekarang == null) {
                            $tahunSebelumnya = explode("-", $baris["tanggal"])[0];
                        } else {
                            $tahunSebelumnya = $tahunSekarang;
                        }
                        $bulanSekarang = explode("-", $baris["tanggal"])[1];
                        $tahunSekarang = explode("-", $baris["tanggal"])[0];
                        if ($bulanSebelumnya == $bulanSekarang && $tahunSebelumnya == $tahunSekarang) {
                            $pemasukanBulanan = $pemasukanBulanan + $baris["jumlah"] + $jumlahSementara;
                            $jumlahSementara = 0;
                        } else { ?>
                            <?php if ($pemasukanBulanan == 0) { ?>
                                <tr>
                                    <th class="text-center"><?= $i; ?></th>
                                    <td>[M] <?= $namaBulan[$bulanSebelumnya - 1]; ?> <?= $tahunSebelumnya; ?></td>
                        ...
                        <?php if ($x == sizeof($unionPengeluaran)) { ?>
                            <?php if ($pemasukanBulanan == 0) {
                                $pemasukanBulanan = $baris["jumlah"];
                            } ?>
                            <tr>
                                <th class="text-center"><?= $i; ?></th>
                                <td>[M] <?= $namaBulan[$bulanSekarang - 1]; ?> <?= $tahunSekarang; ?></td>
                                <td>Rp<?= number_format($pemasukanBulanan, 2, ",", "."); ?></td>
                            </tr>
                            <?php $i++;
                            $totalPemasukan = $totalPemasukan + $pemasukanBulanan;
                ...
                <tbody>
                    ...
                    <?php foreach ($unionPengeluaran as $row) : ?>
                        ...
                        if ($bulanBefore == $bulanNow && $tahunBefore == $tahunNow) {
                            ...
                            <?php $jumlahSementara = $row["jumlah"]; ?>
                            <!-- <?php foreach ($anggota2 as $baris) :
                                        $bulanSekarang = explode("-", $baris["tanggal"])[1];
                                        $tahunSekarang = explode("-", $baris["tanggal"])[0];
                                        if ($baris["kode"] == "Kredit" && $bulanBefore == $bulanSekarang && $tahunBefore == $tahunSekarang) {
                                            $angka = $baris["jumlah"];
                                            $pengeluaranBulanan = $pengeluaranBulanan + $angka;
                                        }
                                    endforeach ?> -->
                            <tr>
                                <th class="text-center"><?= $i; ?></th>
                            ...
                            <?php $i++;
                            $totalPengeluaran = $totalPengeluaran + $pengeluaranBulanan;
                            $pengeluaranBulanan = 0; ?>
                        <?php } ?>
                        <?php if ($x == sizeof($unionPengeluaran)) { ?>
                            <!-- <?php foreach ($anggota2 as $baris) :
                    ...
                    <?php foreach ($anggota2 as $baris) :
                        ...
                        $tahunSekarang = explode("-", $baris["tanggal"])[0];
                        if ($bulanSebelumnya == $bulanSekarang && $tahunSebelumnya == $tahunSekarang) {
                            $pengeluaranBulanan = $pengeluaranBulanan + $baris["jumlah"] + $jumlahSementara;
                            $jumlahSementara = 0;
                        } else { ?>
                            <?php if ($pengeluaranBulanan == 0) { ?>
                                <tr>
                            ...
                            $satu = $baris["jumlah"]; ?>
                        <?php } ?>
                        <?php if ($x == sizeof($anggota2)) { ?>
                            <?php if ($pengeluaranBulanan == 0) {
                                $pengeluaranBulanan = $baris["jumlah"];
                            } ?>
                            <tr>
                                <th class="text-center"><?= $i; ?></th>
                                <td>[M] <?= $namaBulan[$bulanSekarang - 1]; ?> <?= $tahunSekarang; ?></td>
